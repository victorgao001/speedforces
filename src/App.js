...
class Text extends React.Component{
    ...
    render(){
        if(this.state.done){
            var s='.'+this.state.text,t='.'+this.state.userText,userOut=[],textOut=[],combinedOut=[],cpm=0;
            if(s.length*t.length<=4194304){
                ...
                for(let i=s.length-1,j=t.length-1,k=dp[i][j][0]>=dp[i][j][1]? 0:1,p=0;i>0||j>0;){
                    ...
                    if(p==0){
                        ...
                        combinedOut.push(<span className="highlight" key={j}>{t[j]}</span>);
                        i--; j--; cpm++;
                    }
                    else if(p==1){
                        userOut.push(<span key={j}>{t[j]}</span>);
                        combinedOut.push(<span style={{background: "red"}} key={j}>{t[j]}</span>);
                        j--;
                    }
                    else{
                        textOut.push(<span key={i}>{s[i]}</span>);
                        combinedOut.push(<span key={i}>{s[i]}</span>);
                        i--;
                    }
                }
                textOut=textOut.reverse(); userOut=userOut.reverse(); combinedOut=combinedOut.reverse();
            }
            else{
                var jj=1;
